...
namespace TheSTAR.GUI
{
    public class HandfulFlyUI : MonoBehaviour
    {
        ...
        public void Fly(int count, Sprite icon, RectTransform endPos, Action endAction)
        {
            flyObjectsCount = count;
            this.endPos = endPos;
            this.endAction = endAction;

            GenerateFlyUIObjects(flyObjectsCount, icon);
            AnimateHandful();
        }

        private void GenerateFlyUIObjects(int count, Sprite icon)
        {
            flyUIObjects = new FlyUIObject[count];
            for (int i = 0; i < count; i++) flyUIObjects[i] = GenerateFlyUIObject(icon);
        }

        private FlyUIObject GenerateFlyUIObject(Sprite icon)
        {
            var flyObject = Instantiate(flyUiPrefab, transform.position, Quaternion.identity, transform);
            flyObject.SetIcon(icon);
            return flyObject;
        }
